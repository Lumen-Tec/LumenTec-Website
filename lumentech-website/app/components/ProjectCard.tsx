...
interface SocialLink {
  type: 'instagram' | 'facebook' | 'twitter' | 'linkedin' | 'whatsapp';
  url: string;
}
...
export default function ProjectCard({
  ...
  // Helper para obtener el icono SVG según el tipo de red social
      ...
      <div className="grid md:grid-cols-12 gap-0">
        ...
        <div className="md:col-span-8 p-8 md:p-10 flex flex-col justify-center">
          ...
          <div className="flex flex-col gap-4">
            {/* Botón del sitio web o badge de sistema interno */}
            <div>
              {websiteUrl ? (
                <a
                  href={websiteUrl}
            ...
            {socialLinks && socialLinks.length > 0 && (
              <div className="flex items-center gap-3">
                <span className="text-sm text-slate-600 font-medium">Contacto:</span>
                <div className="flex gap-2">
                  {socialLinks.map((link, idx) => (
                    <a
                      ...
                      target="_blank"
                      rel="noopener noreferrer"
                      className="flex items-center justify-center w-10 h-10 rounded-lg bg-slate-100 text-slate-600 hover:bg-indigo-100 hover:text-indigo-600 transition-all hover:scale-110 hover:shadow-md"
                      title={link.type}
                    >
